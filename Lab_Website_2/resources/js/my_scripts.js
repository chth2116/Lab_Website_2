...
function changeColor(color) {
  //alert("Function called with parameter")
...
function loadStatsPage() {
  ...
  for (var i = 2; i < table.rows.length; i++) {
    ...
    if (table.rows[i].cells[2].innerHTML < table.rows[i].cells[3].innerHTML) {
      ...
      num_loses = num_loses + 1;
    }

    document.getElementById('wins').innerHTML = num_wins;
    document.getElementById('losses').innerHTML = num_loses;
    //console.log(i);
  }
}
...
function loadPlayersPage() {
  ...
  for (var player_i = 0; player_i < players.length; player_i++) {
    selectionText +=
      '<a href="#" onclick="switchPlayers(' +
      player_i +
      ')"> ' +
      players[player_i].name +
      ' </a><br>';
  }
...
function switchPlayers(playerNum) {
  ...
  document.getElementById('g_played').innerHTML =
    players[playerNum].games_played;
  document.getElementById('p_yards').innerHTML = players[playerNum].pass_yards;
  document.getElementById('avg_p_yards').innerHTML = parseInt(
    players[playerNum].pass_yards / players[playerNum].games_played
  );
  document.getElementById('r_yards').innerHTML =
    players[playerNum].rushing_yards;
  document.getElementById('avg_r_yards').innerHTML = parseInt(
    players[playerNum].rushing_yards / players[playerNum].games_played
  );
  document.getElementById('rec_yards').innerHTML =
    players[playerNum].receiving_yards;
  document.getElementById('avg_rec_yards').innerHTML = parseInt(
    players[playerNum].receiving_yards / players[playerNum].games_played
  );
  // document.getElementById('r_yards').innerHTML =
  //   players[playerNum].receiving_yards;
  // document.getElementById('avg_r_yards').innerHTML =
  //   players[playerNum].;
